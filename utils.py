...
class LoadDataset(Data.Dataset):
    def __init__(self, VIS, IR, cropsize, gray=True):
        self.VIS =  glob(os.path.join(VIS, '*.*'))
        self.IR = glob(os.path.join(IR, '*.*'))
        # self.resize = resize
    ...
    def __getitem__(self, index):
        VIS = Image.open(self.VIS[index])
        IR = Image.open(self.IR[index])
