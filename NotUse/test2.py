...
from utils import mkdir, CustomVisionDataset_test, gradient, hist_similar
...
# from sewar.full_ref import msssim, pnsr
...
def test():
    ...
    checkpoint = torch.load('New_model/train_result/Training_best.pkl')
    model.load_state_dict(checkpoint['weight'])

    root = 'New_model/Original_test_image'
    sub1 = 'VIS'
    sub2 = 'IR'    
    dataset = CustomVisionDataset_test(root, sub1, sub2)
    ...
    MSE_fun = nn.MSELoss()
    SSIM_fun = SSIM(data_range=1.0).to(device)
    ...
    def test(model):
        ...
        for index in tqdms:
            ...
            msssim_loss = (ssim_loss1 + ssim_loss2)/2
